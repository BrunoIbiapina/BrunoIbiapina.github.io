  ...
  if (cache && cache.data === hoje) {
    ...
    fetch(URL)
      ...
      .then(data => {
        ...
        localStorage.setItem("noticiasTec", JSON.stringify({
          data: hoje,
          noticias: noticias
        }));
    ...
    lista.forEach(noticia => {
      container.innerHTML += `
        <div class="bg-zinc-900 p-4 rounded-lg shadow-md flex flex-col justify-between min-w-[300px] snap-center">
          <img src="${noticia.image_url || 'https://via.placeholder.com/300'}" alt="notícia" class="w-full h-40 object-cover rounded mb-4">
          <h3 class="text-white font-bold text-lg">${noticia.title}</h3>
          <p class="text-gray-400 text-sm mt-2">${noticia.description || ''}</p>
          <a href="${noticia.link}" target="_blank" class="text-primary font-bold mt-2 hover:underline">Leia mais</a>
        </div>
      `;
    });
  }
